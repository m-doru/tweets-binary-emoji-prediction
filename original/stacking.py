...
def create_folds_predictions(classifiers, X_train, y_train):
    ...
    for train_indices, test_indices in kFold.split(X_train):
        ...
        for index, classifier in enumerate(classifiers):
            ...
            classifiers_fold_predictions[index] = classifiers_fold_predictions[index] + predictions
            folds_real_labels += y_test_fold

    return np.array(classifiers_fold_predictions), np.array(folds_real_labels)
...
def create_folds_and_test_predictions(classifiers, X_train, y_train, X_test, y_test):
    ...
    for index, classifier in enumerate(classifiers):
        ...
        classifiers_test_predictions[index] = predictions_test
    ...
    return classifiers_fold_predictions, folds_real_labels, np.array(classifiers_test_predictions), np.array(test_real_labels)
